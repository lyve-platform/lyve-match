...
VALUES ('billing', jsonb_build_object('payments_enabled', false))
ON CONFLICT (key) DO NOTHING;

INSERT INTO public.role_permissions (role, permission) VALUES
  ('super_admin','settings.billing')
ON CONFLICT DO NOTHING;

CREATE OR REPLACE FUNCTION public.payments_enabled()
RETURNS boolean
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT coalesce((value ->> 'payments_enabled')::boolean, false)
  FROM public.app_settings WHERE key = 'billing';
$$;

REVOKE ALL ON FUNCTION public.payments_enabled() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.payments_enabled() TO authenticated, service_role;
...
DECLARE actor uuid;
        previous boolean;
BEGIN
  actor := public.require_permission('settings.billing');
