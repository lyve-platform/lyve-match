...
function isAlreadyOwned(message: string): boolean {
  ...
  // but some Billing v7 purchase paths surface DEVELOPER_ERROR (5) or the
  // generic ERROR (6). All must replay ownership; the server is authoritative.
  return /purchase_failed:(5|6|7)\b/.test(message) || /already\s*(own|subscrib|purchas)/i.test(message);
}
