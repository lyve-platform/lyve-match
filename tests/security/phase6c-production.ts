...
async function appleResponse(
  ...
  const signed = await signJws({
    ...
    expiresDate:
      opts.expiresDate === null ? undefined : (opts.expiresDate ?? NOW + 29 * 86_400_000),
...
async function main() {
  ...
  await withStoreEnv({}, () => {
    check(
      "A1 with no store credentials a sandbox deployment falls back only to the test rail",
      appleRail() === "hmac",
    );
    check(
      "A2 default deployment environment is sandbox, never production",
  ...
  await withStoreEnv(APPLE_PROD_CREDS, () => {
    ...
    check(
      "A3 production credentials resolve only in a production deployment",
      config.ok && config.config.environment === "production",
      config,
    );
    check(
      "A4 production Apple traffic targets the production App Store host",
      config.ok && config.config.apiBase === "https://api.storekit.itunes.apple.com",
    );
    check(
      "A5 a production deployment accepts no attacker-choosable trust anchor",
      appleTrustedRoots().length === 0,
    );
    check(
      "A6 production runs the real store rail, never the HMAC test rail",
      appleRail() === "api",
    );
    check(
      "A7 store mode reports production only with production credentials",
  ...
  await withStoreEnv(
    { ...APPLE_PROD_CREDS, APPLE_SANDBOX_ROOT_FINGERPRINTS: fixtures.rogueRootFingerprint },
    () => {
      check(
        "A8 a root-fingerprint override is ignored in production",
        appleTrustedRoots().length === 0,
      );
    },
  );
  ...
  await withStoreEnv(
    { ...APPLE_SANDBOX_CREDS, ...APPLE_PROD_CREDS, LYVE_STORE_ENVIRONMENT: "production" },
    async () => {
      check(
        "A9 sandbox credentials present in a production deployment are a misconfiguration",
        hasMisplacedAppleCredentials(),
      );
      check(
        "A10 a misconfigured deployment refuses to verify purchases",
        (await verifyStorePurchase("apple", "x")).ok === false,
      );
      const verification = await verifyStorePurchase("apple", "x");
      check(
        "A11 the refusal is MISCONFIGURED, not a fallback",
        !verification.ok && verification.reason === "MISCONFIGURED",
        verification,
      );
      check(
        "A12 a misconfigured deployment reports the store as disabled",
        storeMode() === "disabled",
        storeMode(),
      );
    },
  ...
  await withStoreEnv(
    { ...APPLE_SANDBOX_CREDS, ...APPLE_PROD_CREDS, LYVE_STORE_ENVIRONMENT: "sandbox" },
    async () => {
      check(
        "A13 production credentials present in a sandbox deployment are a misconfiguration",
        hasMisplacedAppleCredentials(),
      );
      const verification = await verifyStoreNotification("apple", "{}", new Headers());
      check(
        "A14 a misconfigured deployment refuses notifications too",
        !verification.ok && verification.reason === "MISCONFIGURED",
        verification,
      );
    },
  ...
  await withStoreEnv({ LYVE_STORE_ENVIRONMENT: "production" }, async () => {
    check(
      "A15 production without credentials trusts nothing (rail none)",
      appleRail() === "none" && googleRail() === "none",
    );
    check(
      "A16 production without credentials is disabled, not sandbox",
      storeMode() === "disabled",
      storeMode(),
    );
    const verification = await verifyStorePurchase("apple", "anything");
    check(
      "A17 an unconfigured production deployment refuses purchases",
      !verification.ok && verification.reason === "NOT_CONFIGURED",
      verification,
    );
  });

  await withStoreEnv(
    { LYVE_STORE_ENVIRONMENT: "production", STORE_SANDBOX_SECRET: "x".repeat(40) },
    async () => {
      ...
      check(
        "A18 the sandbox HMAC rail cannot operate a production deployment",
        !verification.ok,
        verification,
      );
      check(
        "A19 a sandbox-signed receipt never yields production access",
        !verification.ok && verification.reason === "NOT_CONFIGURED",
        verification,
      );
    },
  ...
  await withStoreEnv({ ...APPLE_PROD_CREDS, APPLE_IAP_PRIVATE_KEY: "not-a-key" }, () => {
    const config = appleConfig();
    check(
      "A20 a malformed production key is rejected, not ignored",
      !config.ok && config.reason === "INVALID_CREDENTIAL",
      config,
    );
    check(
      "A21 an invalid credential disables the store rather than downgrading it",
      appleRail() === "none",
    );
  });

  await withStoreEnv(
    { LYVE_STORE_ENVIRONMENT: "production", APPLE_IAP_ISSUER_ID: "only-one" },
    () => {
      const config = appleConfig();
      check(
        "A22 a partially configured production store is not configured",
        !config.ok && config.reason === "NOT_CONFIGURED",
        config,
      );
    },
  );
  ...
  await withStoreEnv(GOOGLE_PROD_CREDS, () => {
    ...
    check(
      "A23 production Play credentials resolve to the production package",
      config.ok && config.config.packageName === "com.lyve.app",
    );
    check(
      "A24 production Play push expectations are pinned",
      config.ok &&
        config.config.pushAudience === GOOGLE_PROD_CREDS.GOOGLE_RTDN_AUDIENCE &&
        config.config.pushServiceAccountEmail ===
          GOOGLE_PROD_CREDS.GOOGLE_RTDN_SERVICE_ACCOUNT_EMAIL,
    );
    check("A25 production Play runs the real API rail", googleRail() === "api");
  });

  await withStoreEnv(
    { ...GOOGLE_PROD_CREDS, GOOGLE_PLAY_SANDBOX_PACKAGE_NAME: "com.lyve.app.test" },
    async () => {
      check(
        "A26 sandbox Play credentials in production are a misconfiguration",
        hasMisplacedGoogleCredentials(),
      );
      const verification = await verifyStorePurchase("google", "token");
      check(
        "A27 the misconfigured Play store refuses purchases",
        !verification.ok && verification.reason === "MISCONFIGURED",
        verification,
      );
    },
  ...
  await withStoreEnv(
    { ...GOOGLE_PROD_CREDS, GOOGLE_PLAY_SERVICE_ACCOUNT_JSON: "{not json" },
    () => {
      const config = googleConfig();
      check(
        "A28 an unparseable service account is rejected",
        !config.ok && config.reason === "INVALID_CREDENTIAL",
        config,
      );
    },
  ...
  await withStoreEnv(
    {
      ...GOOGLE_PROD_CREDS,
      GOOGLE_PLAY_SERVICE_ACCOUNT_JSON: JSON.stringify({ client_email: "a@b.c" }),
    },
    () => {
      const config = googleConfig();
      check(
        "A29 a service account without a key is rejected",
        !config.ok && config.reason === "INVALID_CREDENTIAL",
        config,
      );
    },
  ...
  await withStoreEnv(APPLE_PROD_CREDS, async () => {
    const notification = JSON.stringify({
      signedPayload: await signJws({ notificationType: "DID_RENEW" }),
    });
    const verification = await verifyStoreNotification("apple", notification, new Headers());
    check(
      "B1 a self-signed Apple notification is refused in production",
  ...
  await withStoreEnv(APPLE_PROD_CREDS, async () => {
    const verification = await verifyStoreNotification(
      "apple",
      JSON.stringify({ notificationType: "DID_RENEW" }),
      new Headers(),
    );
    check(
      "B2 an unsigned Apple notification is refused in production",
      !verification.ok && verification.reason === "MISSING_SIGNATURE",
      verification,
    );
  });
  ...
  await withStoreEnv(APPLE_PROD_CREDS, async () => {
    const headers = new Headers({
      "x-lyve-store-signature": "sha256=deadbeef",
      "x-lyve-store-timestamp": String(Math.floor(Date.now() / 1000)),
    });
    const verification = await verifyStoreNotification("apple", "{}", headers);
    check(
      "B3 the sandbox HMAC header is meaningless in production",
      !verification.ok,
      verification,
    );
  });
  ...
  await withStoreEnv(GOOGLE_PROD_CREDS, async () => {
    ...
    check(
      "B4 an RTDN push without an OIDC token is refused",
      !verification.ok && verification.reason === "MISSING_SIGNATURE",
      verification,
    );
  });
  ...
  {
    const push = await authenticatePubSubPush(
      new Headers({ authorization: "Bearer not.a.token" }),
      {
    ...
    check(
      "B5 an unverifiable push token is refused",
      !push.ok && push.reason === "INVALID_TOKEN",
      push,
    );
  }
  {
    ...
    check(
      "B6 a production RTDN endpoint without pinned push expectations is closed, not open",
      !push.ok && push.reason === "PUSH_NOT_CONFIGURED",
      push,
    );
  }
  ...
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse(),
      applePseudoProd,
      NOW,
    );
    check(
      "C1 a production Apple purchase verifies from the store API",
      parsed.ok && parsed.snapshot.environment === "production",
      parsed,
    );
    check(
      "C2 the verified purchase maps to a known LYVE plan",
      parsed.ok && productFor("apple", parsed.snapshot.productId)?.planCode === "premium_monthly",
    );
    check(
      "C3 access derives from store status, not the client",
      parsed.ok && parsed.snapshot.lifecycle.status === "active",
    );
  }
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse({ environment: "Sandbox" }),
      applePseudoProd,
      NOW,
    );
    check(
      "C4 a sandbox transaction is refused by a production deployment",
      !parsed.ok && parsed.reason === "WRONG_ENVIRONMENT",
      parsed,
    );
  }
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse({ bundleId: "com.attacker.app" }),
      applePseudoProd,
      NOW,
    );
    check(
      "C5 a transaction for another app is refused",
      !parsed.ok && parsed.reason === "WRONG_ENVIRONMENT",
      parsed,
    );
  }
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse({ productId: "com.attacker.free" }),
      applePseudoProd,
      NOW,
    );
    check(
      "C6 an unknown production product is refused",
      !parsed.ok && parsed.reason === "UNKNOWN_PRODUCT",
      parsed,
    );
  }
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse({ status: 5 }),
      applePseudoProd,
      NOW,
    );
    check(
      "C7 a revoked production purchase revokes access",
      parsed.ok && parsed.snapshot.lifecycle.revoke === true,
      parsed,
    );
  }
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse({ status: 4 }),
      applePseudoProd,
      NOW,
    );
    check(
      "C8 a production billing-grace purchase keeps access as past_due",
      parsed.ok && parsed.snapshot.lifecycle.status === "past_due",
      parsed,
    );
  }
  {
    const parsed = await parseAppleSubscriptionResponse(
      await appleResponse({ revocationDate: NOW - 1000 }),
      applePseudoProd,
      NOW,
    );
    check(
      "C9 a refunded production purchase revokes access",
      parsed.ok &&
        parsed.snapshot.lifecycle.revoke === true &&
        parsed.snapshot.lifecycle.reason === "api_revoked",
      parsed,
    );
  }
  {
    ...
    check(
      "C10 a malformed production response is refused",
      !parsed.ok && parsed.reason === "MALFORMED_RESPONSE",
      parsed,
    );
  }
  for (const [status, expected] of [
    ...
    check(
      `C11 an Apple ${status} response is a refusal (${expected})`,
      !result.ok && result.reason === expected,
      result,
    );
  }
  {
    const result = await fetchAppleSubscriptionState("../../etc/passwd", {
      config: applePseudoProd,
      fetchImpl: stubFetch(200, {}),
      now: NOW,
    });
    check(
      "C12 a malformed transaction id never reaches the store",
...

  {
    ...
    check(
      "C13 a production Play purchase verifies",
      parsed.ok &&
        parsed.snapshot.environment === "production" &&
        parsed.snapshot.lifecycle.status === "active",
      parsed,
    );
  }
  {
    ...
    check(
      "C14 a Play test purchase is refused in production",
      !parsed.ok && parsed.reason === "WRONG_ENVIRONMENT",
      parsed,
    );
  }
  {
    ...
    check(
      "C15 a revoked Play purchase revokes access",
      parsed.ok && parsed.snapshot.lifecycle.revoke === true,
      parsed,
    );
  }
  {
    const parsed = parseGoogleSubscriptionResponse(
      googleResponse({
        lineItems: [{ productId: GOOGLE_PRODUCT, autoRenewingPlan: { autoRenewEnabled: false } }],
      }),
      "t",
      googlePseudoProd,
    );
    check(
      "C16 auto-renew off keeps production access until period end",
      parsed.ok &&
        parsed.snapshot.lifecycle.cancelAtPeriodEnd === true &&
        parsed.snapshot.lifecycle.revoke === false,
      parsed,
    );
  }
  {
    const parsed = parseGoogleSubscriptionResponse(
      googleResponse({ lineItems: [{ productId: "attacker.free" }] }),
      "t",
      googlePseudoProd,
    );
    check(
      "C17 an unknown Play product is refused in production",
      !parsed.ok && parsed.reason === "UNKNOWN_PRODUCT",
      parsed,
    );
  }
  {
    ...
    check(
      "C18 a denied Play read is never treated as valid",
  ...
  }
  {
    ...
    check(
      "C19 the production Play read is scoped to the production package",
      result.ok && requested.includes("com.lyve.app/purchases/subscriptionsv2/tokens/prod-token"),
      requested,
    );
  }
  {
    ...
    check(
      "C20 without an override, Apple JWS must chain to the pinned Apple root",
      !verified.ok,
      verified,
    );
  }
  ...
  const link = await admin.rpc(
    "billing_link_store_purchase",
    linkArgs(alice.id, appleRef) as never,
  );
  check(
    "D1 a verified production purchase binds to the purchasing account",
    String(link.data) === "linked",
    link,
  );
  ...
  check(
    "D2 re-presenting the same purchase is idempotent",
    String(relink.data) === "already_owned",
    relink,
  );
  ...
  check(
    "D3 the purchase cannot be transferred to another account",
  ...
  const stale = await applyEvent({
    store: "apple",
    eventId: `${appleRef}-stale`,
    eventAt: new Date(NOW - 3_600_000).toISOString(),
    purchaseRef: appleRef,
    status: "expired",
    reason: "EXPIRED",
  });
  ...
  const cancel = await applyEvent({
    store: "apple",
    eventId: `${appleRef}-cancel`,
    eventAt: new Date(NOW + 1000).toISOString(),
    purchaseRef: appleRef,
    status: "canceled",
    cancelAtPeriodEnd: true,
    reason: "DID_CHANGE_RENEWAL_STATUS:AUTO_RENEW_DISABLED",
  });
  ...
  const expire = await applyEvent({
    store: "apple",
    eventId: `${appleRef}-expire`,
    eventAt: new Date(NOW + 2000).toISOString(),
    purchaseRef: appleRef,
    status: "expired",
  ...
  const resubscribe = await applyEvent({
    store: "apple",
    eventId: `${appleRef}-resub`,
    eventAt: new Date(NOW + 3000).toISOString(),
    purchaseRef: appleRef,
    status: "active",
    reason: "SUBSCRIBED",
  });
  check(
    "D14 a resubscription restores Premium to the same owner",
    resubscribe === "applied" && (await isPremium(alice)),
  );

  const refund = await applyEvent({
    store: "apple",
    eventId: `${appleRef}-refund`,
    eventAt: new Date(NOW + 4000).toISOString(),
    purchaseRef: appleRef,
    status: "expired",
    revoke: true,
    reason: "REFUND",
  ...
  check(
    "D17 a refunded purchase still cannot be claimed by another account",
  ...
  const unlinked = await applyEvent({
    store: "google",
    eventId: `p6c-orphan-${stamp}`,
    eventAt: new Date().toISOString(),
    purchaseRef: `p6c-unknown-${stamp}`,
    status: "active",
    reason: "RENEW",
  });
  check(
    "D18 an event for an unlinked purchase grants nobody access",
    unlinked === "unlinked",
    unlinked,
  );
  ...
  check(
    "E1 unchanged production state reconciles to the same event id",
    first!.eventId === second!.eventId,
  );
  check(
    "E2 changed production state reconciles to a new event id",
    reconciliationEventId({ ...snapshot, stateToken: "2:api_expired:y" }) !== first!.eventId,
  );
  check("E3 reconciliation event ids carry no purchase token", !first!.eventId.includes(appleRef));

  const recon1 = await applyEvent({
    store: "apple",
    eventId: first!.eventId,
    eventAt: new Date(NOW + 5000).toISOString(),
    purchaseRef: appleRef,
    status: "active",
    reason: "reconciliation",
  });
  const recon2 = await applyEvent({
    store: "apple",
    eventId: second!.eventId,
    eventAt: new Date(NOW + 6000).toISOString(),
    purchaseRef: appleRef,
    status: "active",
    reason: "reconciliation",
  });
  ...
  check(
    "E6 reconciliation restores access to the rightful owner only",
    (await isPremium(alice)) && !(await isPremium(bob)),
  );

  /* ============ F. Rate limiting, monitoring, redaction ============ */

  check(
    "F1 production webhooks stay rate limited",
    RATE_LIMITS.webhook.limit > 0 && RATE_LIMITS.webhook.windowSeconds > 0,
  );
  check(
    "F2 signature failures are limited more tightly than traffic",
    RATE_LIMITS.webhookFailure.limit < RATE_LIMITS.webhook.limit,
  );
  check(
    "F3 account linking is the tightest surface",
    RATE_LIMITS.link.limit < RATE_LIMITS.webhookFailure.limit,
  );

  const digest = refDigest("prod-purchase-token-abc");
  check(
    "F4 purchase tokens are recorded only as a short digest",
    digest.length === 16 && !digest.includes("prod"),
  );
  ...
  check(
    "F6 monitoring drops receipts, tokens, payloads and credentials",
    !serialized.includes("SECRET"),
    serialized,
  );
  check(
    "F7 monitoring keeps the operational fields it needs",
    serialized.includes("apple") && serialized.includes("INVALID_SIGNATURE"),
  );
  ...
  const visitorAlerts = await createClient(url, publishableKey, { auth: { persistSession: false } })
    ...
    .limit(1);
  check(
    "F8 visitors cannot read the production alert stream",
    Boolean(visitorAlerts.error) || (visitorAlerts.data ?? []).length === 0,
    visitorAlerts.data,
  );

  const memberAlerts = await alice.client.from("store_alerts").select("id").limit(1);
  check(
    "F9 members cannot read the alert stream",
    Boolean(memberAlerts.error) || (memberAlerts.data ?? []).length === 0,
    memberAlerts.data,
  );

  const memberLedger = await alice.client.from("billing_events").select("id").limit(1);
  check(
    "F10 members cannot read the billing event ledger",
  ...
  check(
    "G1 this deployment is NOT running production purchases",
    storeMode() !== "production",
    storeMode(),
  );
  check("G2 no Apple production credentials are connected", !process.env["APPLE_IAP_PRIVATE_KEY"]);
  check(
    "G3 no Google production service account is connected",
