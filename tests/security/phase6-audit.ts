...
async function main() {
  ...
  check("store mode is sandbox — no production store credentials connected", storeMode() === "sandbox", storeMode());
  ...
  check("sandbox verification secret is configured and long enough", Boolean(secret && secret.length >= 16));
  check("product catalogue only maps known LYVE plans", STORE_PRODUCTS.every((p) => p.planCode.startsWith("premium_")));
  check("apple product resolves server-side", productFor("apple", APPLE_PRODUCT)?.planCode === "premium_monthly");
  check("google product resolves server-side", productFor("google", GOOGLE_PRODUCT)?.planCode === "premium_monthly");
  ...
  const payloadSwap = buildSandboxReceipt("not-the-real-secret-value", { store: "apple", purchase_ref: appleRef, product_id: APPLE_PRODUCT, environment: "sandbox" });
  check("Apple receipt signed with a wrong secret rejected", (await verifyStorePurchase("apple", payloadSwap)).ok === false);
  check("unsigned Apple receipt rejected", (await verifyStorePurchase("apple", "just-a-string")).ok === false);
  ...
  check("unknown store id rejected", (await verifyStorePurchase("amazon", validApple)).ok === false);
  ...
  check("verified Google purchase carries the purchase token", validGoogle.ok && validGoogle.purchase.purchaseRef === googleRef);
  check("forged Google purchase token rejected", (await verifyStorePurchase("google", `${googleReceipt(googleRef).split(".")[0]}.deadbeef`)).ok === false);
  check("Google receipt with unknown subscription id rejected", (await verifyStorePurchase("google", googleReceipt(googleRef, { product_id: "free_forever" }))).ok === false);
  check("Google receipt claiming production rejected", (await verifyStorePurchase("google", googleReceipt(googleRef, { environment: "production" }))).ok === false);
  check("Google receipt cannot be presented as an Apple purchase", (await verifyStorePurchase("apple", googleReceipt(googleRef))).ok === false);
  ...
  check("ASSN body swapped under a valid signature rejected", (await verifyStoreNotification("apple", bodySwap, swapHeaders)).ok === false);

  const malformed = "{not json";
  check("malformed ASSN body rejected", (await verifyStoreNotification("apple", malformed, signedHeaders(malformed))).ok === false);

  const rtdnBody = googleNotification({ id: `rtdn-${stamp}-a`, notificationType: 4, purchaseRef: googleRef });
  check("signed RTDN verifies", (await verifyStoreNotification("google", rtdnBody, signedHeaders(rtdnBody))).ok === true);
  check("RTDN with forged signature rejected", (await verifyStoreNotification("google", rtdnBody, new Headers({ [STORE_TIMESTAMP_HEADER]: String(Math.floor(Date.now() / 1000)), [STORE_SIGNATURE_HEADER]: "sha256=ff" }))).ok === false);
  const rtdnStale = await verifyStoreNotification("google", rtdnBody, signedHeaders(rtdnBody, staleTs));
  check("replayed (stale-timestamp) RTDN rejected", !rtdnStale.ok && rtdnStale.reason === "STALE_TIMESTAMP");

  /* ================= 5. Payload normalisation guards ================ */
  check("ASSN with unknown notification type rejected", !normalizeAppleEvent(JSON.parse(appleNotification({ id: "x", type: "FREE_PREMIUM_PLEASE", purchaseRef: appleRef }))).ok);
  check("ASSN with unknown product rejected", !normalizeAppleEvent(JSON.parse(appleNotification({ id: "x", type: "SUBSCRIBED", purchaseRef: appleRef, productId: "com.attacker.free" }))).ok);
  check("ASSN claiming production rejected", !normalizeAppleEvent(JSON.parse(appleNotification({ id: "x", type: "SUBSCRIBED", purchaseRef: appleRef, environment: "Production" }))).ok);
  check("ASSN without a transaction id rejected", !normalizeAppleEvent({ notificationUUID: "x", notificationType: "SUBSCRIBED", data: { productId: APPLE_PRODUCT, environment: "sandbox" } }).ok);
  check("ASSN carries no LYVE user id field", !JSON.stringify(normalizeAppleEvent(JSON.parse(notifBody))).includes("profile_id"));
  check("RTDN with unknown notification type rejected", !normalizeGoogleEvent(JSON.parse(googleNotification({ id: "x", notificationType: 999, purchaseRef: googleRef }))).ok);
  check("RTDN without purchase token rejected", !normalizeGoogleEvent({ messageId: "x", subscriptionNotification: { notificationType: 4, subscriptionId: GOOGLE_PRODUCT } }).ok);
  check("RTDN epoch-millis event time normalises", normalizeGoogleEvent(JSON.parse(googleNotification({ id: "x", notificationType: 4, purchaseRef: googleRef, eventTimeMillis: 1_700_000_000_000 }))).ok === true);
  ...
  check("Apple auto-renew disabled → canceled at period end", appleLifecycle("DID_CHANGE_RENEWAL_STATUS", "AUTO_RENEW_DISABLED")?.cancelAtPeriodEnd === true);
  check("Apple auto-renew re-enabled → active", appleLifecycle("DID_CHANGE_RENEWAL_STATUS", "AUTO_RENEW_ENABLED")?.status === "active");
  check("Apple grace period → past_due", appleLifecycle("DID_FAIL_TO_RENEW", "GRACE_PERIOD")?.status === "past_due");
  check("Apple billing retry → past_due", appleLifecycle("DID_FAIL_TO_RENEW", null)?.status === "past_due");
  check("Apple grace expiry → expired", appleLifecycle("GRACE_PERIOD_EXPIRED", null)?.status === "expired");
  check("Apple EXPIRED → expired without revocation", appleLifecycle("EXPIRED", null)?.revoke === false);
  ...
  check("Google canceled (3) → canceled at period end", googleLifecycle(3)?.cancelAtPeriodEnd === true);
  ...
  check("Google mapping table has no unexpected revocations", Object.entries(GOOGLE_NOTIFICATION_TYPES).filter(([, v]) => v.revoke).map(([k]) => k).join() === "12");
  check("Google unknown notification type has no mapping", googleLifecycle(42) === null);
  check("grace period (past_due) still grants access", statusGrantsAccess("past_due", null) === true);
  check("expired grants no access", statusGrantsAccess("expired", new Date(Date.now() + 864e5).toISOString()) === false);
  check("cancellation keeps access until period end", statusGrantsAccess("canceled", new Date(Date.now() + 864e5).toISOString()) === true);
  check("cancellation past period end grants no access", statusGrantsAccess("canceled", new Date(Date.now() - 864e5).toISOString()) === false);
  ...
  const first = await admin.rpc("billing_link_store_purchase", linkArgs(alice.id, appleRef) as never);
  check("first link binds the purchase to the authenticated account", String(first.data) === "linked", first.error?.message);

  const repeat = await admin.rpc("billing_link_store_purchase", linkArgs(alice.id, appleRef) as never);
  check("re-linking the same purchase to the same account is idempotent", String(repeat.data) === "already_owned");

  const transfer = await admin.rpc("billing_link_store_purchase", linkArgs(bob.id, appleRef) as never);
  check("purchase cannot be transferred to a second account", String(transfer.data) === "owned_by_other");

  const { data: ownerRow } = await admin.from("store_purchases").select("profile_id, environment, status").eq("purchase_ref", appleRef).single();
  ...
  const { data: auditRows } = await admin.from("store_purchase_audit").select("outcome, attempted_profile_id, owner_profile_id, purchase_ref_hash").order("created_at", { ascending: false }).limit(5);
  ...
  check("audit stores a hash, never the raw store token", !(auditRows ?? []).some((r) => r.purchase_ref_hash.includes(appleRef)));
  check("audit hash is a sha-256 hex digest", /^[0-9a-f]{64}$/.test(conflict?.purchase_ref_hash ?? ""));

  const googleLink = await admin.rpc("billing_link_store_purchase", linkArgs(bob.id, googleRef, "google") as never);
  check("a Google purchase binds independently to its own account", String(googleLink.data) === "linked");

  /* ================= 8. Client forgery of ownership ================= */
  const forgedLink = await alice.client.rpc("billing_link_store_purchase", linkArgs(bob.id, `forged-${stamp}`) as never);
  check("members cannot call the purchase-linking routine", Boolean(forgedLink.error));
  const anonLink = await anon.rpc("billing_link_store_purchase", linkArgs(alice.id, `anon-${stamp}`) as never);
  check("anonymous callers cannot call the purchase-linking routine", Boolean(anonLink.error));
  const memberApply = await alice.client.rpc("billing_apply_store_event", { p_provider: "apple", p_purchase_ref: appleRef, p_event_id: "x", p_event_at: new Date().toISOString(), p_status: "active", p_plan_code: "premium_monthly", p_interval: "month", p_currency: "USD", p_period_start: null, p_period_end: null, p_cancel_at_period_end: false, p_entitlements: PREMIUM_ENTITLEMENTS } as never);
  check("members cannot apply store lifecycle events", Boolean(memberApply.error));

  const memberInsert = await alice.client.from("store_purchases").insert({ provider: "apple", purchase_ref: `self-${stamp}`, profile_id: alice.id, product_id: APPLE_PRODUCT, plan_code: "premium_monthly" } as never);
  check("members cannot insert store purchase rows", Boolean(memberInsert.error));
  const memberUpdate = await alice.client.from("store_purchases").update({ status: "active" }).eq("purchase_ref", appleRef);
  check("members cannot update store purchase rows", Boolean(memberUpdate.error) || (await admin.from("store_purchases").select("status").eq("purchase_ref", appleRef).single()).data?.status !== "active");
  const memberDelete = await alice.client.from("store_purchases").delete().eq("purchase_ref", appleRef);
  check("members cannot delete store purchase rows", Boolean(memberDelete.error) || Boolean((await admin.from("store_purchases").select("id").eq("purchase_ref", appleRef).maybeSingle()).data));

  /* ================= 9. Lifecycle application ======================= */
  const t0 = new Date(Date.now() - 60_000).toISOString();
  const purchased = await applyEvent({ store: "apple", eventId: `evt-${stamp}-1`, eventAt: t0, purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "active" });
  check("verified purchase event grants Premium to the owning account", purchased.outcome === "applied", purchased.error?.message);
  ...
  const { data: subRow } = await admin.from("subscriptions").select("profile_id, purchase_source, provider, status").eq("provider", "apple").eq("provider_subscription_id", appleRef).maybeSingle();
  ...
  check("re-using an Apple notification id is rejected by the ledger", claim1.error?.code === "23505");
  ...
  check("concurrent duplicate store events: exactly one is accepted", concurrent.filter((r) => !r.error).length === 1);

  /* out-of-order */
  const renewAt = new Date().toISOString();
  const renewed = await applyEvent({ store: "apple", eventId: `evt-${stamp}-2`, eventAt: renewAt, purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "active" });
  check("renewal event applies", renewed.outcome === "applied");
  const outOfOrder = await applyEvent({ store: "apple", eventId: `evt-${stamp}-3`, eventAt: t0, purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "expired" });
  ...
  const { data: staleAudit } = await admin.from("store_purchase_audit").select("outcome").eq("event_id", `evt-${stamp}-3`).maybeSingle();
  check("stale event is recorded in the audit trail", staleAudit?.outcome === "stale_event");

  /* unlinked purchase */
  const unlinked = await applyEvent({ store: "apple", eventId: `evt-${stamp}-4`, eventAt: new Date().toISOString(), purchaseRef: `never-linked-${stamp}`, productId: APPLE_PRODUCT, status: "active" });
  check("event for an unlinked purchase grants nothing", unlinked.outcome === "unlinked");

  /* grace period / billing retry */
  const grace = await applyEvent({ store: "apple", eventId: `evt-${stamp}-5`, eventAt: new Date(Date.now() + 1000).toISOString(), purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "past_due" });
  ...
  /* cancellation */
  const canceled = await applyEvent({ store: "apple", eventId: `evt-${stamp}-6`, eventAt: new Date(Date.now() + 2000).toISOString(), purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "canceled", cancelAtPeriodEnd: true, periodEnd: new Date(Date.now() + 10 * 864e5).toISOString() });
  ...
  const { data: aliceRestore } = await alice.client.from("store_purchases").select("purchase_ref, profile_id");
  check("restore returns only the caller's own purchases", (aliceRestore ?? []).every((r) => r.profile_id === alice.id));
  check("restore surfaces the caller's linked purchase", (aliceRestore ?? []).some((r) => r.purchase_ref === appleRef));
  const { data: bobSees } = await bob.client.from("store_purchases").select("purchase_ref").eq("purchase_ref", appleRef);
  ...
  const { data: switchedRows } = await switched.from("store_purchases").select("purchase_ref").eq("purchase_ref", appleRef);
  check("switching accounts on a shared device does not carry Premium across", (switchedRows ?? []).length === 0);
  check("the second account is still not Premium", !(await isPremium(bob)));

  /* expiration */
  const expired = await applyEvent({ store: "apple", eventId: `evt-${stamp}-7`, eventAt: new Date(Date.now() + 3000).toISOString(), purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "expired", periodEnd: new Date(Date.now() - 864e5).toISOString() });
  check("expiry event applies", expired.outcome === "applied");
  check("expired subscription loses Premium", !(await isPremium(alice)));

  /* refund / revocation */
  const reAdd = await applyEvent({ store: "apple", eventId: `evt-${stamp}-8`, eventAt: new Date(Date.now() + 4000).toISOString(), purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "active" });
  check("resubscribe after expiry applies", reAdd.outcome === "applied");
  check("resubscribe restores Premium", await isPremium(alice));
  const refunded = await applyEvent({ store: "apple", eventId: `evt-${stamp}-9`, eventAt: new Date(Date.now() + 5000).toISOString(), purchaseRef: appleRef, productId: APPLE_PRODUCT, status: "expired", revoke: true, reason: "refund" });
  ...
  const { data: revokedRow } = await admin.from("store_purchases").select("status, revoked_at").eq("purchase_ref", appleRef).single();
  check("purchase row is marked revoked", revokedRow?.status === "expired" && Boolean(revokedRow?.revoked_at));
  const { data: revokeAudit } = await admin.from("store_purchase_audit").select("outcome").eq("event_id", `evt-${stamp}-9`).maybeSingle();
  check("revocation is recorded in the audit trail", revokeAudit?.outcome === "revoked");
  const { data: revokedEnt } = await admin.from("entitlements").select("revoked_at, revoke_reason").eq("profile_id", alice.id).eq("key", "premium").order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  check("a refunded purchase still cannot move to another account", String(postRefundTransfer.data) === "owned_by_other");
  ...
  const ledgerRewrite = await admin.from("billing_events").update({ provider_event_id: `rewritten-${stamp}` }).eq("provider_event_id", `evt-${stamp}-1`);
  check("a ledger row's store event identity cannot be rewritten", Boolean(ledgerRewrite.error));
  const payloadRewrite = await admin.from("billing_events").update({ payload_summary: { premium: true } }).eq("provider_event_id", `evt-${stamp}-1`);
  check("a ledger row's recorded payload cannot be rewritten", Boolean(payloadRewrite.error));
  const ledgerDelete = await admin.from("billing_events").delete().eq("provider_event_id", `evt-${stamp}-1`);
  check("billing events cannot be deleted", Boolean(ledgerDelete.error));
  const auditUpdate = await admin.from("store_purchase_audit").update({ outcome: "linked" }).eq("event_id", `evt-${stamp}-9`);
  check("store purchase audit is append-only (no updates)", Boolean(auditUpdate.error));
  const auditDelete = await admin.from("store_purchase_audit").delete().eq("event_id", `evt-${stamp}-9`);
  ...
  const { data: ledgerRow } = await admin.from("billing_events").select("payload_summary").eq("provider_event_id", `rtdn-${stamp}-dup`).maybeSingle();
  check("ledger stores no raw store payload", !JSON.stringify(ledgerRow?.payload_summary ?? {}).includes("purchaseToken"));
  ...
  await applyEvent({ store: "apple", eventId: `evt-${stamp}-c1`, eventAt: new Date().toISOString(), purchaseRef: carolRef, productId: APPLE_PRODUCT, status: "active" });
  check("purchase links for the deletion fixture", Boolean((await admin.from("store_purchases").select("id").eq("purchase_ref", carolRef).maybeSingle()).data));
  await admin.auth.admin.deleteUser(carol.id);
  const { data: afterDelete } = await admin.from("store_purchases").select("id").eq("purchase_ref", carolRef).maybeSingle();
  check("deleting the account removes its store purchase binding", afterDelete === null);
  const { data: auditSurvives } = await admin.from("store_purchase_audit").select("id").eq("event_id", `evt-${stamp}-c1`).maybeSingle();
  check("audit trail survives account deletion", Boolean(auditSurvives));
  const relink = await admin.rpc("billing_link_store_purchase", linkArgs(bob.id, carolRef) as never);
  check("after deletion the store purchase can be re-linked to a new account", String(relink.data) === "linked");
}
