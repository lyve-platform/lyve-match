...

{
  ...
  const spliced = await signJws(transactionPayload(), [
    fixtures.rogueLeafB64,
    fixtures.intermediateB64,
    fixtures.rootB64,
  ]);
  ...
  check(
    "A6 alg=none is refused",
    !algResult.ok && algResult.reason !== "MALFORMED"
      ? algResult.reason === "UNSUPPORTED_ALG"
      : algResult.ok === false,
    algResult,
  );
  ...
  check(
    "A7 symmetric alg substitution is refused",
    !hsResult.ok && hsResult.reason === "UNSUPPORTED_ALG",
    hsResult,
  );
  ...
  check(
    "A8 missing x5c is refused",
    !noChainResult.ok && noChainResult.reason === "MISSING_CHAIN",
    noChainResult,
  );
  ...
  check(
    "A10 expired certificates are refused",
    !future.ok && future.reason === "CERT_EXPIRED",
    future,
  );

  const malformed = await verifyAppleJws("not-a-jws", { trustedRootFingerprints: TEST_ROOTS });
  check(
    "A11 malformed token is refused",
    !malformed.ok && malformed.reason === "MALFORMED",
    malformed,
  );

  check(
    "A12 default trust anchor is the pinned Apple Root CA - G3 fingerprint",
    APPLE_ROOT_CA_G3_FINGERPRINT.length === 64 &&
      APPLE_ROOT_CA_G3_FINGERPRINT !== fixtures.rootFingerprint,
  );
...

{
  ...
  check(
    "B3 partial credentials are NOT configured (never partial trust)",
    !partial.ok && partial.reason === "NOT_CONFIGURED",
    partial,
  );
  ...
  check(
    "B4 production credentials in a sandbox deployment are detected",
    hasMisplacedAppleCredentials(),
  );
  const misplaced = appleConfig();
  check(
    "B5 misplaced credentials fail closed",
    !misplaced.ok && misplaced.reason === "CREDENTIAL_MISPLACED",
    misplaced,
  );
  ...
  check(
    "B7 sandbox credentials resolve to the sandbox API host",
    ok.ok && ok.config.apiBase.includes("storekit-sandbox"),
    ok.ok ? ok.config.apiBase : ok,
  );
  check("B8 sandbox credentials select the API rail", appleRail() === "api");
  check(
    "B9 test trust anchors are accepted in sandbox",
    appleTrustedRoots().includes(fixtures.rootFingerprint),
  );

  process.env["LYVE_STORE_ENVIRONMENT"] = "production";
  check(
    "B10 test trust anchors are ignored in a production deployment",
    appleTrustedRoots().length === 0,
  );
  check(
    "B11 sandbox credentials in a production deployment fail closed",
    appleConfig().ok === false,
  );
  ...
  check(
    "B12 a non-PEM private key is rejected",
    !badKey.ok && badKey.reason === "INVALID_CREDENTIAL",
    badKey,
  );
  ...
  check(
    "B13 malformed service-account JSON is rejected",
    !badJson.ok && badJson.reason === "INVALID_CREDENTIAL",
    badJson,
  );
  ...
  check(
    "B14 google sandbox credentials resolve",
    gc.ok && gc.config.packageName === "com.lyve.app",
  );
...

{
  ...
  check(
    ...
    active.ok &&
      active.snapshot.lifecycle.status === "active" &&
      active.snapshot.purchaseRef === "2000000999000111",
    active,
  );

  const revoked = await parseAppleSubscriptionResponse(await appleResponseBody(5), APPLE_CONFIG);
  check(
    "C2 revoked status revokes entitlements",
    revoked.ok && revoked.snapshot.lifecycle.revoke,
    revoked,
  );
  ...
  check(
    "C3 a revocation date beats an 'active' status",
    refunded.ok && refunded.snapshot.lifecycle.revoke,
    refunded,
  );

  const retry = await parseAppleSubscriptionResponse(await appleResponseBody(3), APPLE_CONFIG);
  check(
    "C4 billing retry keeps the account in past_due",
    retry.ok && retry.snapshot.lifecycle.status === "past_due",
  );

  const cancelled = await parseAppleSubscriptionResponse(
    await appleResponseBody(1, {}, GOOD_CHAIN, 0),
    APPLE_CONFIG,
  );
  check(
    "C5 auto-renew off is canceled-at-period-end, not immediate loss",
    cancelled.ok &&
      cancelled.snapshot.lifecycle.status === "canceled" &&
      cancelled.snapshot.lifecycle.cancelAtPeriodEnd,
    cancelled,
  );
  ...
  check(
    "C7 unknown product is refused",
    !unknown.ok && unknown.reason === "UNKNOWN_PRODUCT",
    unknown,
  );
  ...
  const forged = await parseAppleSubscriptionResponse(
    await appleResponseBody(1, {}, ROGUE_CHAIN),
    APPLE_CONFIG,
  );
  check(
  ...
  check(
    "C10 an empty response is refused",
    !empty.ok && empty.reason === "MALFORMED_RESPONSE",
    empty,
  );

  const unsupported = await parseAppleSubscriptionResponse(
    await appleResponseBody(99),
    APPLE_CONFIG,
  );
  check(
    "C11 an unknown status code is refused",
    !unsupported.ok && unsupported.reason === "UNSUPPORTED_STATE",
    unsupported,
  );
}

{
  ...
  const result = await fetchAppleSubscriptionState("2000000999000111", {
    fetchImpl: stub,
    config: APPLE_CONFIG,
  });
  check(
    "C12 transport calls the sandbox host only",
    seenUrl.startsWith("https://api.storekit-sandbox.itunes.apple.com/inApps/v1/subscriptions/"),
    seenUrl,
  );
  ...
  check(
    "C15 client assertion is ES256 with the configured key id",
    jwt?.header["alg"] === "ES256" && jwt?.header["kid"] === "ABCD1234",
  );
  check(
  ...
  for (const [status, reason] of codes) {
    ...
    const out = await fetchAppleSubscriptionState("2000000999000111", {
      fetchImpl: failing,
      config: APPLE_CONFIG,
    });
    check(
      `C17 HTTP ${status} maps to ${reason} (never to trust)`,
      !out.ok && out.reason === reason,
      out,
    );
  }
  ...
  const netFail = await fetchAppleSubscriptionState("2000000999000111", {
    fetchImpl: thrown,
    config: APPLE_CONFIG,
  });
  check(
    "C18 a network failure is an upstream error, not an approval",
    !netFail.ok && netFail.reason === "UPSTREAM_ERROR",
  );

  const injected = await fetchAppleSubscriptionState("../../../etc/passwd", {
    fetchImpl: stub,
    config: APPLE_CONFIG,
  });
  check(
    "C19 a malformed transaction id never reaches the store",
    !injected.ok && injected.reason === "NOT_FOUND",
    injected,
  );
}
...

{
  ...
  for (const [label, claims, reason] of cases) {
    ...
    check(
      `D2 push token with ${label} is refused (${reason})`,
      !out.ok && out.reason === reason,
      out,
    );
  }
  ...
  check(
    "D3 an unknown signing key is refused",
    !unknownKidResult.ok && unknownKidResult.reason === "UNKNOWN_KEY",
  );
  ...
  check(
    "D4 tampering with push claims breaks the signature",
    !tamperedResult.ok && tamperedResult.reason === "BAD_SIGNATURE",
  );
  ...
  const authed = await authenticatePubSubPush(pushHeaders(valid), {
...

{
  const payload = {
    ...
    subscriptionNotification: {
      ...
      subscriptionId: "lyve_premium_monthly",
    },
  };
  ...
  check(
    "D10 an envelope without messageId is refused",
    !decodePubSubEnvelope(JSON.stringify({ message: { data: "e30" } })).ok,
  );
  check(
    "D11 an envelope with non-JSON data is refused",
    !decodePubSubEnvelope(JSON.stringify({ message: { data: "%%%", messageId: "1" } })).ok,
  );
}

{
  ...
  check(
    "D12 active Play subscription maps to active",
    active.ok && active.snapshot.lifecycle.status === "active",
  );

  const grace = parseGoogleSubscriptionResponse(
  ...
  const paused = parseGoogleSubscriptionResponse(
    body({ subscriptionState: "SUBSCRIPTION_STATE_PAUSED" }),
    "tok",
    GOOGLE_CONFIG,
  );
  ...
  check(
    "D15 a revoked purchase revokes entitlements",
    revoked.ok && revoked.snapshot.lifecycle.revoke,
  );

  const cancelled = parseGoogleSubscriptionResponse(
    body({}, { autoRenewingPlan: { autoRenewEnabled: false } }),
    "tok",
    GOOGLE_CONFIG,
  );
  check(
    "D16 auto-renew off keeps access to period end",
    cancelled.ok && cancelled.snapshot.lifecycle.cancelAtPeriodEnd,
  );

  const production = parseGoogleSubscriptionResponse(
    { ...body(), testPurchase: undefined },
    "tok",
    GOOGLE_CONFIG,
  );
  check(
    "D17 a production purchase is refused by a sandbox deployment",
    !production.ok && production.reason === "WRONG_ENVIRONMENT",
    production,
  );

  const unknown = parseGoogleSubscriptionResponse(
    body({}, { productId: "lyve_premium_lifetime" }),
    "tok",
    GOOGLE_CONFIG,
  );
  check(
    "D18 an unknown Play product is refused",
    !unknown.ok && unknown.reason === "UNKNOWN_PRODUCT",
  );

  const unsupported = parseGoogleSubscriptionResponse(
    body({ subscriptionState: "SUBSCRIPTION_STATE_UNSPECIFIED" }),
    "tok",
    GOOGLE_CONFIG,
  );
  check(
    "D19 an unspecified state is refused",
    !unsupported.ok && unsupported.reason === "UNSUPPORTED_STATE",
  );

  const malformed = parseGoogleSubscriptionResponse({}, "tok", GOOGLE_CONFIG);
  check(
    "D20 a malformed Play response is refused",
    !malformed.ok && malformed.reason === "MALFORMED_RESPONSE",
  );
}

{
  ...
  const stub = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response(
      JSON.stringify({
        ...
        testPurchase: {},
        lineItems: [
          { productId: "lyve_premium_monthly", expiryTime: new Date(NOW + 1000).toISOString() },
        ],
      }),
      { status: 200 },
    );
  ...
  check(
    "D22 exchange uses the JWT bearer grant",
    tokenBody.includes("grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Ajwt-bearer"),
  );
  ...
  const state = await fetchGoogleSubscriptionState("tok-abc", {
    fetchImpl: stub,
    config: GOOGLE_CONFIG,
  });
  ...
  const deniedResult = await fetchGoogleSubscriptionState("tok-abc", {
    fetchImpl: denied,
    config: GOOGLE_CONFIG,
  });
  check(
    "D26 a denied Play read is never treated as valid",
    !deniedResult.ok && deniedResult.reason === "UNAUTHORIZED",
  );

  const badCreds = (async () => new Response("{}", { status: 401 })) as typeof fetch;
  const badCredsResult = await fetchGoogleSubscriptionState("tok-abc", {
    fetchImpl: badCreds,
    config: GOOGLE_CONFIG,
  });
  check(
    "D27 rejected service-account credentials fail closed",
    !badCredsResult.ok && badCredsResult.reason === "UNAUTHORIZED",
  );
}
...
{
  check(
    "E1 apple status 1 with auto-renew is active",
    appleStatusLifecycle(1, true)?.status === "active",
  );
  check(
    "E2 apple status 2 is expired without revocation",
    appleStatusLifecycle(2)?.status === "expired" && appleStatusLifecycle(2)?.revoke === false,
  );
  ...
  check(
    "E6 unknown play state maps to nothing",
    googleStateLifecycle("SUBSCRIPTION_STATE_NEW") === null,
  );
  check(
    "E7 play pending is incomplete, not active",
    googleStateLifecycle("SUBSCRIPTION_STATE_PENDING")?.status === "incomplete",
  );
  ...
  check(
    "E10 reconciliation event ids are namespaced and carry no token",
    idA.startsWith("recon:apple:") && !idA.includes(snapshot.purchaseRef),
  );

  const event = snapshotToEvent(snapshot);
  check(
    "E11 snapshot converts to a verified event",
    event?.purchaseRef === snapshot.purchaseRef && event?.eventType.startsWith("reconciliation:"),
  );
  check(
    "E12 an unknown product cannot be reconciled",
    snapshotToEvent({ ...snapshot, productId: "com.lyve.unknown" }) === null,
  );
...

{
  ...
  check(
    "F1 purchase references are recorded as a short digest",
    digest.length === 16 && !digest.includes(token),
  );
  ...
  check(
    "F4 monitoring drops receipts and tokens",
    !("receipt" in details) && !("purchase_token" in details) && !("authorization" in details),
    details,
  );
  check(
    "F5 monitoring keeps only bounded operational fields",
    details["store"] === "apple" &&
      String(details["reason"]).length === 120 &&
      details["hits"] === 3,
  );
  check(
    "F6 webhook rate limit is finite",
    RATE_LIMITS.webhook.limit > 0 && RATE_LIMITS.webhook.windowSeconds > 0,
  );
  check(
    "F7 member linking is throttled more tightly than webhooks",
    RATE_LIMITS.link.limit < RATE_LIMITS.webhook.limit,
  );
}
...

{
  ...
  for (let i = 0; i < 4; i += 1) {
    const { data } = await admin.rpc("store_rate_limit_hit", {
      p_bucket: bucket,
      p_limit: 3,
      p_window_seconds: 60,
    });
  ...
  const { data: shortWindow } = await admin.rpc("store_rate_limit_hit", {
    p_bucket: `${bucket}:c`,
    p_limit: 0,
    p_window_seconds: 60,
  });
  check(
    "G4 a nonsensical limit denies rather than allows",
    (shortWindow as { allowed?: boolean })?.allowed === false,
  );

  const anonLimiter = await anon.rpc("store_rate_limit_hit", {
    p_bucket: bucket,
    p_limit: 3,
    p_window_seconds: 60,
  });
  ...
  const anonAlert = await anon.rpc("store_raise_alert", {
    p_kind: "x",
    p_severity: "info",
    p_fingerprint: "y",
  });
  ...
  check(
    "G11 alerts are append-only, even for the service role",
    deletion.error !== null,
  ...
  const { data: purchaseColumns, error: columnError } = await admin
    ...
    .limit(1);
  check(
    "G16 purchases track their last reconciliation",
    columnError === null,
    columnError?.message ?? purchaseColumns,
  );
}
